...
class Ulogin extends Module
{
	...
	public $errors = array();
	private $hooks = array('displayNav', 'displayHeader', 'displayFooter', 'DisplayCustomerAccount',
		'DisplayMyAccountBlock');
	private $ulogin_default_options = array('display' => 'small',
		'providers' => 'vkontakte,odnoklassniki,mailru,facebook', 'hidden' => 'other',
		'fields' => 'first_name,last_name,email,photo,photo_big', 'optional' => 'phone', 'redirect_uri' => '',);

	private $ulogin_options = array('ulogin_label' => 'Войти с помощью:', 'ulogin_id1' => '', 'ulogin_id2' => '',
		'ulogin_check_mail' => false, 'id_default_group' => '',);
	...
	public function __construct()
	{
		...
		if (!Configuration::get('ulogin_default_options') && !Configuration::get('ulogin_options')) $this->warning =
			$this->l('Упс, что-то пошло не так!');
	}
	...
	{
		$ulogin_gr =
			Db::getInstance()->executeS('SELECT id_group FROM '._DB_PREFIX_.'group_lang WHERE name = "'.$this->name.
				'"');
		if (!$ulogin_gr)
		{
			$res = Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.
				'group` (`id_group`, `date_add`, `date_upd`) VALUES (NULL, NOW(), NOW())');
			$last_id = Db::getInstance()->Insert_ID();

			$languages = Db::getInstance()->executeS('SELECT id_lang, iso_code FROM `'._DB_PREFIX_.'lang`');

			$sql = '';
			foreach ($languages as $lang) $sql .= '('.(int)$last_id.', '.(int)$lang['id_lang'].', "'.$this->name.'"),';
			$sql = Tools::substr($sql, 0, Tools::strlen($sql) - 1);
			$res &= Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.
				'group_lang` (`id_group`, `id_lang`, `name`) VALUES '.$sql);
			// Add shop association
			$res &= Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.
				'group_shop` (`id_group`, `id_shop`) (SELECT '.(int)$last_id.', `value` FROM `'._DB_PREFIX_.
				'configuration` WHERE `name` = \'PS_SHOP_DEFAULT\')');
			...
			$res &= Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.
				'category_group` (`id_category`, `id_group`) (SELECT `id_category`, '.(int)$last_id.' FROM `'.
				_DB_PREFIX_.'category_group` WHERE `id_group` = 3)');
			$controllers = Module::getModulesInstalled();
			$id_modules = array();
			foreach ($controllers as $key => $id_module) $id_modules[$key] = $id_module['id_module'];
			$shops = Shop::getShops(true, null, true);
			if ($res = Group::addModulesRestrictions($last_id, $id_modules, $shops)) return $res;
		}
		return true;
	}
	...
	public function install()
	{
		if (!parent::install() || !$this->registerHooks() ||
			!Configuration::updateValue('ulogin_default_options', serialize($this->ulogin_default_options)) ||
			!Configuration::updateValue('ulogin_options', serialize($this->ulogin_options))) return false;
		...
		// исполняем запрос
		if (!Db::getInstance()->Execute(trim($sql))) return false;

		//создаём таблицу для группы uLogin
		if (!$this->addulogingroup()) return false;
	...
	{
		if (!parent::uninstall() || !Configuration::deleteByName('ulogin_default_options') ||
			!Configuration::deleteByName('ulogin_options') || !$this->unregisterHooks()) return false;
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
			...
			if (count($this->errors)) foreach ($this->errors as $err) $output .= $this->displayError($err);
			else
			{
	...
	public function displayForm()
	{
		...
		$check_mail =
			(int)Configuration::get('PS_CUSTOMER_CREATION_EMAIL') == 1 ? $this->l('включен') : $this->l('выключен');
		...
		$fields_form = array();
		$fields_form[0]['form'] =
			array('legend' => array('title' => $this->l('Настройки плагина uLogin'), 'icon' => 'icon-cogs'),
				'input' => array(array('type' => 'free',
					'label' => '<span class="h3">ID uLogin`а из <a href="http://ulogin.ru/lk.php"
						target="_blank">Личного кабинета</a></span>', 'col' => $ulogin_col,
					'desc' => '<span style="color: red">'.$this->l('Внимание!').'</span> '.
						$this->l('Перед установкой uLogin ID, в настройках виджета').' '.$this->l('в').
						' <a href="http://ulogin.ru/lk.php" target="_blank">'.$this->l('Личном кабинете').'</a> '.
						$this->l('необходимо указать').' <b>'.$this->l('email').'</b> '.
						$this->l('в возвращаемых полях').$this->l('т.к. по умолчанию этот параметр отключён.'),
					'name' => 'text',),
					array('type' => 'text', 'label' => $this->l('uLogin ID форма входа для панели навигации:'),
						'name' => 'ulogin_id1', 'required' => false, 'col' => $ulogin_col,
						'desc' => $this->l('Идентификатор виджета для панели навигации
						 (хук Nav). Пустое поле - виджет по умолчанию'),),
					array('type' => 'text', 'label' => $this->l('uLogin ID общая форма:'), 'name' => 'ulogin_id2',
						'col' => $ulogin_col,
						'desc' => $this->l('Идентификатор виджета для любого хука. Пустое
						поле - виджет по умолчанию')),
					array('type' => 'free', 'label' => '<span class="h3">'.$this->l('Другие параметры').'</span>',
						'name' => 'text',),
					array('type' => 'text', 'label' => $this->l('Текст:'), 'name' => 'ulogin_label',
						'col' => $ulogin_col, 'desc' => $this->l('Текст типа "Войти с помощью:"')),
					array('type' => 'checkbox',
						'label' => $this->l('Отправлять письмо при регистрации новому пользователю:'),
						'name' => 'ulogin_check_mail', 'col' => $ulogin_col,
						'desc' => $this->l('В настройках магазина этот параметр - ').$check_mail,
						'values' => array('query' => array(array('name' => 'mail', 'val' => '1',
							'checked' => 'checked'),),)),
					array('type' => 'select', 'label' => $this->l('Группа клиентов по умолчанию'),
						'name' => 'id_default_group',
						'options' => array('query' => $groups, 'id' => 'id_group', 'name' => 'name'),
						'col' => $ulogin_col,
						'desc' => $this->l('Для пользователей авторизованных с помощью uLogin эта группа
						 будет группой по умолчанию.')),),
				'submit' => array('title' => $this->l('Сохранить настройки'), 'class' => 'button'));
		...
		$helper->toolbar_btn = array('save' => array('desc' => $this->l('Сохранить'),
		...
		$ulogin_gr =
			Db::getInstance()->getValue('SELECT id_group FROM '._DB_PREFIX_.'group_lang WHERE name = "'.$this->name.
				'"');
		...
		$helper->fields_value['id_default_group'] =
			$ulogin_options['id_default_group'] == '' ? $ulogin_gr : $ulogin_options['id_default_group'];
	...
	public function uloginCurrentPageUrl()
	{
		...
		if (!empty($page_url)) return $page_url;
		else if (Tools::getValue('backurl')) return Tools::getValue('backurl');

		$page_url = 'http';
		if (isset($_SERVER['HTTPS']))
			if ($_SERVER['HTTPS'] == 'on') $page_url .= 's';
		$page_url .= '://';
		if ($_SERVER['SERVER_PORT'] != '80') $page_url .= $_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].
			$_SERVER['REQUEST_URI'];
		else
	...
	public function drawPanel($place = 0, $with_label = true)
	{
		...
		if ($default_panel)
		{
			...
			if ($ul_options['display'] != 'window') $panel .= ' data-ulogin="'.$x_ulogin_params.'"></div>';
				...
				$panel .= ' data-ulogin="'.$x_ulogin_params.
					'" href="#"><img src="https://ulogin.ru/img/button.png" width=187 height=30 alt="МультиВход"/></div>';
		}
		else
	...
	{
		if (!Context::getContext()->customer->isLogged()) return $this->drawPanel(0);
		return false;
	}
	...
	{
		$currcontroller = Tools::strtolower(get_class($this->context->controller));
		if ($currcontroller == 'authcontroller') $this->drawPanel(1);
		if ($currcontroller == 'uloginprofilemodulefrontcontroller')
		{
	...
	public function uloginGetUserFromToken($token = false)
	{
		...
		if ($token)
		{
			...
			$request = 'http://ulogin.ru/token.php?token='.$token.'&host='.$_SERVER['HTTP_HOST'].'&data='.
				base64_encode(Tools::jsonEncode($data));
			if (function_exists('curl_init'))
			{
				...
				elseif (function_exists('file_get_contents') && ini_get('allow_url_fopen')) $response =
					Tools::file_get_contents($request);
			}
			else
			{
	...
	public function uloginRegistrationUser($u_user, $in_db = 0)
	{
		...
		{
			$this->errors = $this->l('Через данную форму выполнить вход/регистрацию невозможно.').'<br/>'.
				$this->l('Сообщите администратору сайта о следующей ошибке:').'<br/>'.
				$this->l('Необходимо указать "email" в возвращаемых полях uLogin');
		...
		if ($in_db == 1) Db::getInstance()->delete(_DB_PREFIX_.'customer_ulogin_table', 'identity = "'.
			urlencode($u_user['identity']).'"');

		$user_id = Db::getInstance()->getValue('SELECT id_customer FROM '._DB_PREFIX_.'customer WHERE email = "'.
			$u_user['email'].'"');
		...
		if (($check_m_user == false) && !$is_logged_in)
		{
			...
			if ($customer->add())
			{
				...
				{
					$ulogin_gr_id =
						Db::getInstance()->getValue('SELECT id_group FROM '._DB_PREFIX_.'group_lang WHERE name = "'.
							$this->name.'"');
					$ulogin_options['id_default_group'] = $ulogin_gr_id;
				}

				$addugr = Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'customer SET id_default_group = "'.
					$ulogin_options['id_default_group'].'" WHERE id_customer = "'.$customer->id.'"');
				$adducustomergr = Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'customer_group SET id_group = "'.
					$ulogin_options['id_default_group'].'" WHERE id_customer = "'.$customer->id.'"');
				if (!$addugr) $this->errors = $this->l('Ошибка при добавлении пользователя в группу ulogin');
				if (!$adducustomergr) $this->errors = $this->l('Ошибка при добавлении пользователя в группу ulogin');
				if (!$this->sendConfirmationMail($customer)) $this->errors = $this->l('Ошибка при отправке письма');
		...
		else
		{
			...
			{
				$this->errors =
					$this->l('Электронный адрес данного аккаунта совпадает с электронным адресом
					 существующего пользователя').
					'<br>'.$this->l('Требуется подтверждение на владение указанным email.');
				$this->context->smarty->assign(array('message' => ('<script src="//ulogin.ru/js/ulogin.js"
				type="text/javascript"></script><script type="text/javascript">uLogin.mergeAccounts("'.
					Tools::getValue('token').'")</script>'), 'ulogin_message' => $this->errors));
			}
			...
			if ($u_user['verified_email'] == 1)
			{
				...
				$other_u = Db::getInstance()->getValue('SELECT identity FROM '._DB_PREFIX_.
					'customer_ulogin_table WHERE userid = "'.$user_id.'"');
				if ($other_u)
				{
					if (!$is_logged_in && !isset($u_user['merge_account']))
					{
						$this->errors =
							$this->l('С данным аккаунтом уже связаны данные из другой социальной сети.').'<br>'.
							$this->l('Требуется привязка новой учётной записи социальной сети к этому
							аккаунту.');
						$this->context->smarty->assign(array('message' => ('<script src="//ulogin.ru/js/ulogin.js"
						type="text/javascript"></script><script type="text/javascript">uLogin.mergeAccounts("'.
							Tools::getValue('token').'","'.$other_u.'")</script>'), 'ulogin_message' => $this->errors));
						return false;
					}
				}
	...
	public function uloginInsertRow($user_id, $identity, $network = '')
	{
		if ($user_id > 0)
		{
			$result = Db::getInstance()->insert('customer_ulogin_table', array('userid' => $user_id,
				'identity' => urlencode($identity), 'network' => $network));

			if ($result != false) return $user_id;
		}
		else
			return false;
		return '';
	...
	public function loginCustomer($u_user, $id_customer)
	{
		...
		$result =
			Db::getInstance()->GetRow('SELECT * FROM '._DB_PREFIX_.'customer WHERE id_customer = "'.$id_customer.'"');
		if (!empty ($result['id_customer']) && ($id_customer == $result['id_customer']))
		{
			...
			foreach ($result as $key => $value)
			{
				if (array_key_exists($key, $customer))
				{
					...
					{
						case 'birthday':
							if ($value == $u_user['bdate']) $customer->birthday = $value;
							else
							{
								if (!empty($u_user['bdate']) && isset($u_user['bdate']))
								{
									$customer->birthday = date('Y-m-d', strtotime($u_user['bdate']));
									Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'customer SET birthday = "'.
										$customer->birthday.'" WHERE id_customer = "'.$id_customer.'"');
								}
							}
							break;
						case 'id_gender':
							...
							if ($value == $id_gender) $customer->id_gender = $value;
							else
							{
								if (!empty($id_gender) && isset($u_user['sex']))
								{
									$customer->id_gender = $id_gender;
									Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'customer SET id_gender = "'.
										$customer->id_gender.'" WHERE id_customer = "'.$id_customer.'"');
								}
							}
							break;
						default:
			...
			$context->cookie->id_compare = isset($context->cookie->id_compare) ? $context->cookie->id_compare :
			...
			if (Configuration::get('PS_CART_FOLLOWING') &&
				...
				$id_cart = (int)Cart::lastNoneOrderedCart($context->customer->id)) $context->cart = new Cart($id_cart);
			else
			{
			...
			if (Tools::getValue('backurl'))
			{
				...
				$ex = explode('&err', $back);
				$back = $ex[0];
				if (!empty($this->errors)) $back .= (Tools::strpos($back, '?') !== false ? '&' : '?').'err='.urlencode($this->errors);
				Tools::redirect($back);
			}
	...
	public function uloginCheckUserId($user_id)
	{
		...
		{
			$this->errors = $this->l('Данный аккаунт привязан к другому пользователю.').'<br/>'.
				$this->l('Вы не можете использовать этот аккаунт');
			$this->context->smarty->assign('ulogin_message', $this->errors);
			return false;
		}
		return true;
	...
	{
		if (!Tools::getIsset('token')) return;  // не был получен токен uLogin
		$s = $this->uloginGetUserFromToken(Tools::getValue('token'));
		if (!$s)
		{
			$this->errors = '<b>'.$this->l('Ошибка работы uLogin:').'</b></br></br>'.
				$this->l('Не удалось получить данные о пользователе с помощью токена.');
			$this->context->smarty->assign('ulogin_message', $this->errors);
			return false;
		}
		...
		$row = Db::getInstance()->getRow('SELECT userid FROM '._DB_PREFIX_.'customer_ulogin_table WHERE identity = "'.
			urlencode($u_user['identity']).'"');
		...
		{
			$wp_user =
				Db::getInstance()->getRow('SELECT id_customer FROM '._DB_PREFIX_.'customer WHERE id_customer = "'.
					$user_id.'"');
			if ($user_id > 0 && $wp_user['id_customer'] > 0) $this->uloginCheckUserId($user_id);
			else
	...
	public function getUloginUserAccountsPanel($user_id = 0)
	{
		...
		$use_cache = false;
		$networks =
			Db::getInstance()->ExecuteS('SELECT network FROM '._DB_PREFIX_.'customer_ulogin_table WHERE userid = '.
				(int)$user_id, $networks = true, $use_cache);

		$output = '';
		if ($networks)
		{
			$output .= '<div id="ulogin_accounts">';
			foreach ($networks as $network) $output .= "<div data-ulogin-network='{$network['network']}'
	...
	protected function sendConfirmationMail($customer)
	{
		...
		if ($uopt['ulogin_check_mail'] == 0) return true;

		return Mail::Send($this->context->language->id, 'account', Mail::l('Добро пожаловать!'), array('{firstname}' => $customer->firstname,
