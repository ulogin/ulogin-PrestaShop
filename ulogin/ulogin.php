...
class ulogin extends Module
{
    ...
    private $uLoginDefaultOptions = array(
        'display' => 'small',
    ...
    private $uLoginOptions = array(
        'ulogin_label' => 'Войти с помощью:',
        'ulogin_id1' => '',
        'ulogin_id2' => '',
        'ulogin_check_mail' => false,
        'id_default_group' => '',
    );
    ...
    public function __construct()
    {
        ...
        if (!Configuration::get('uLoginDefaultOptions')
            && !Configuration::get('uLoginOptions')
        )
    ...
    function add_ulogin_group()
    {
        ...
        if (!$ulogin_gr) {
            ...
            if ($ress = Group::addModulesRestrictions($last_id, $id_modules, $shops))
                return $res;
        }
        return true;
    }
    ...
    public function install()
    {
        if (!parent::install() ||
            !$this->registerHooks() ||
            !Configuration::updateValue('uLoginDefaultOptions', serialize($this->uLoginDefaultOptions)) ||
            !Configuration::updateValue('uLoginOptions', serialize($this->uLoginOptions))
        )
            return false;
    ...
    {
        if (!parent::uninstall() ||
            !Configuration::deleteByName('uLoginDefaultOptions') ||
            !Configuration::deleteByName('uLoginOptions') ||
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submit' . $this->name)) {
            $uLoginOptions['ulogin_id1'] = strval(Tools::getValue('ulogin_id1'));
            $uLoginOptions['ulogin_id2'] = strval(Tools::getValue('ulogin_id2'));
            $uLoginOptions['ulogin_label'] = strval(Tools::getValue('ulogin_label'));
            $uLoginOptions['ulogin_check_mail'] = strval(Tools::getValue('ulogin_check_mail_'));
            $uLoginOptions['id_default_group'] = strval(Tools::getValue('id_default_group'));
            ...
            if (count($this->errors)) {
                ...
                Configuration::updateValue('uLoginOptions', serialize($uLoginOptions));
    ...
    public function displayForm() //рисуем страницу настроек uLogin в админке
    {
        // Получаем язык сайта, выбранный поумолчанию
        ...
        $fields_form[0]['form'] = array(
            'legend' => array(
                'title' => $this->l('Настройки плагина uLogin'),
                'icon' => 'icon-cogs'
            ),
            'input' => array(
                array(
                    'type' => 'free',
                    'label' => "<span class='h3'>ID uLogin'а из <a href='http://ulogin.ru/lk.php' target='_blank'>Личного кабинета</a></span>",
                    'col' => $ulogin_col,
                    'desc' => "<span style='color: red'>" . $this->l("Внимание!") . "</span>" . ' ' . $this->l("Перед установкой uLogin ID, в настройках виджета") . ' ' .
                        $this->l("в") . " <a href='http://ulogin.ru/lk.php' target='_blank'>" . $this->l("Личном кабинете") .
                        "</a>" . ' ' . $this->l("необходимо указать") . " <b>" . $this->l("email") . "</b> " . $this->l("в возвращаемых полях") .
                        $this->l("т.к. по умолчанию этот параметр отключён."),
                    'name' => 'text',
                ),
                array(
                    'type' => 'text',
                    'label' => $this->l('uLogin ID форма входа для панели навигации:'),
                    'name' => 'ulogin_id1',
                    'required' => false,
                    'col' => $ulogin_col,
                    'desc' => $this->l('Идентификатор виджета для панели навигации (хук Nav). Пустое поле - виджет по умолчанию'),
                ),
                array(
                    'type' => 'text',
                    'label' => $this->l('uLogin ID общая форма:'),
                    'name' => 'ulogin_id2',
                    'col' => $ulogin_col,
                    'desc' => $this->l('Идентификатор виджета для любого хука. Пустое поле - виджет по умолчанию')
                ),
                array(
                    'type' => 'free',
                    'label' => '<span class="h3">' . $this->l("Другие параметры") . '</span>',
                    'name' => 'text',
                ),
                array(
                    'type' => 'text',
                ...
                array(
                    'type' => 'select',
                    'label' => $this->l('Группа клиентов по умолчанию'),
                    ...
                    'col' => $ulogin_col,
                    'desc' => $this->l('Для пользователей авторизованных с помощью uLogin эта группа будет группой по умолчанию.')
                ),
            ),
            'submit' => array(
                'title' => $this->l('Сохранить настройки'),
                'class' => 'button'
            )
        );
        ...
        $uLoginOptions = unserialize(Configuration::get('uLoginOptions'));
        $helper->fields_value['ulogin_id1'] = $uLoginOptions['ulogin_id1'];
        $helper->fields_value['ulogin_id2'] = $uLoginOptions['ulogin_id2'];
        $helper->fields_value['ulogin_label'] = $uLoginOptions['ulogin_label'];
        $helper->fields_value['ulogin_check_mail_'] = ($uLoginOptions['ulogin_check_mail'] == '1' ? true : false);
        $helper->fields_value['id_default_group'] = $uLoginOptions['id_default_group'] == '' ? $ulogin_gr : $uLoginOptions['id_default_group'];
     ...
     * Возвращает back url
     */
    public function ulogin_current_page_url()
    {
        $pageURL = trim(Tools::getValue('back'));
        if (!empty($pageURL)) {
            return $pageURL;
        } else {
    ...
    public function draw_panel($place = 0, $with_label = true)
    {
        $uLoginOptions = unserialize(Configuration::get('uLoginOptions'));
        $uLoginDefaultOptions = unserialize(Configuration::get('uLoginDefaultOptions'));
        $default_panel = false;

        switch ($place) {
            case 0:
                $uloginID = $uLoginOptions['ulogin_id1'];
                break;
            case 1:
                $uloginID = $uLoginOptions['ulogin_id2'];
                break;
            default:
                $uloginID = $uLoginOptions['ulogin_id1'];
        }
        if (empty($uloginID)) {
            $ulOptions = $uLoginDefaultOptions;
            $default_panel = true;
        }

        $panel = $with_label ? '<div class="ulogin_label">' . $uLoginOptions['ulogin_label'] . '&nbsp;</div>' : '';
        $plusone = $this->ulogin_current_page_url();
    ...
    public function hookDisplayFooter($params)
    {
        ...
        if ($currcontroller == 'uloginprofilemodulefrontcontroller') {
            $this->draw_panel(1, false);
            $this->get_ulogin_user_accounts_panel();
        }
        $this->ulogin_parse_request();
    }
    ...
    {
        if (!is_array($u_user)) {
            $this->errors = $this->l("Ошибка работы uLogin: Данные о пользователе содержат неверный формат.");
            return false;
        }
        ...
        if (!isset($u_user['identity'])) {
            $this->errors = $this->l("Ошибка работы uLogin: В возвращаемых данных отсутствует переменная 'identity'");
            return false;
        }
        return true;
    }
    ...
    function ulogin_registration_user($u_user, $in_db = 0)
    {
        if (!isset($u_user['email'])) {
            $this->errors = $this->l('Через данную форму выполнить вход/регистрацию невозможно.') .
                '<br/>' .
                $this->l('Сообщите администратору сайта о следующей ошибке:') .
                '<br/>' . $this->l('Необходимо указать "email" в возвращаемых полях uLogin');
        ...
        if ($in_db == 1) {
            $result = Db::getInstance()->delete(_DB_PREFIX_ . "customer_ulogin_table", "identity = '" . urlencode($u_user['identity']) . "'");
        }
        ...
        $isLoggedIn = Context::getContext()->customer->isLogged();

        if (($check_m_user == false) && !$isLoggedIn) { // отсутствует пользователь с таким email в базе PrestaShop и он не залогинен -> регистрация
            $user_pass = Tools::passwdGen();
            $customer = new CustomerCore();
            $customer->firstname = $u_user ['first_name'];
            $customer->lastname = $u_user ['last_name'];
            if (isset($u_user['sex'])) { //выбор пола, если есть
                switch ($u_user['sex']) {
                    case 0:
            ...
            if ($customer->add()) {
                ...
                if (empty($uLoginOptions['id_default_group'])) {
                    $ulogin_gr_id = Db::getInstance()->getValue("SELECT id_group FROM " . _DB_PREFIX_ . "group_lang WHERE name = '" . $this->name . "'");
                    $uLoginOptions['id_default_group'] = $ulogin_gr_id;
                }

                $addugr = Db::getInstance()->execute("UPDATE " . _DB_PREFIX_ . "customer SET id_default_group = '" . $uLoginOptions['id_default_group'] . "' WHERE id_customer = '" . $customer->id . "'");
                $adducustomergr = Db::getInstance()->execute("UPDATE " . _DB_PREFIX_ . "customer_group SET id_group = '" . $uLoginOptions['id_default_group'] . "' WHERE id_customer = '" . $customer->id . "'");
                if (!$addugr)
                ...
                $this->ulogin_insert_row($customer->id, $u_user['identity'], $network);
                return $customer->id;
            }
            ...
            if (!isset($u_user["verified_email"]) || intval($u_user["verified_email"]) != 1) {
                $this->errors = $this->l("Электронный адрес данного аккаунта совпадает с электронным адресом существующего пользователя") .
                    "<br>" .
                    $this->l("Требуется подтверждение на владение указанным email.");
                $this->context->smarty->assign(array(
                    'message' => ("<script src='//ulogin.ru/js/ulogin.js'  type='text/javascript'></script><script type='text/javascript'>uLogin.mergeAccounts('"
                        . $_POST['token'] .
                        "')</script>"),
                    'ulogin_message' => $this->errors
                ));
            }
            ...
            if (intval($u_user["verified_email"]) == 1) {
                ...
                $this->ulogin_insert_row($user_id, $u_user['identity'], $network);
                return $user_id;
            }
        }
        return false;
     ...
     */
    function login_customer($u_user, $id_customer)
    {
        ...
        if (!empty ($result ['id_customer']) && ($id_customer == $result ['id_customer'])) {
            ...
            foreach ($result as $key => $value) {
                if (array_key_exists($key, $customer)) {
                    //проверка отличаются ли данные базы от данных uLogin, если да и не нулевые, обновляем
                    switch ($key) {
                        case 'birthday':
            ...
            if (Tools::getValue('backurl')) {
                ...
                if (!empty($this->errors)) {
                    $back .= '?err=' . urlencode($this->errors);
                }
                Tools::redirect($back);
            }
    ...
    function ulogin_check_user_id($user_id)
    {
        ...
        if (($current_user > 0) && ($user_id > 0) && ($current_user != $user_id)) {
            $this->errors = $this->l("Данный аккаунт привязан к другому пользователю.") .
                '<br/>' .
    ...
    function ulogin_parse_request()
    {
        ...
        $s = $this->ulogin_get_user_from_token($_POST['token']);
        if (!$s) {
        ...
        $check = $this->ulogin_check_token_error($u_user);
        if (!$check) {
            $this->context->smarty->assign('ulogin_message', $this->errors);
            return false;
        }
        ...
        if (isset($user_id)) {
            ...
            if ($user_id > 0 && $wp_user['id_customer'] > 0) {
                $this->ulogin_check_user_id($user_id);
            } else {
                $user_id = $this->ulogin_registration_user($u_user, 1);
            }
        } else {
            $user_id = $this->ulogin_registration_user($u_user);
        }
        if ($user_id > 0) {
            $this->login_customer($u_user, $user_id);
        }
    ...
    function get_ulogin_user_accounts_panel($user_id = 0)
    {
        ...
        $networks = Db::getInstance()->ExecuteS('SELECT network FROM ' . _DB_PREFIX_ . 'customer_ulogin_table WHERE userid = ' . (int)$user_id, $networks = true, $use_cache = false);
    ...
    /**
     * Высылает подтверждение по почте при регистрации пользователя, если стоит галочка в настройках uLogina
     * @param $customer
     * @return bool
     */
    protected
    function sendConfirmationMail($customer)
    {
        $uopt = unserialize(Configuration::get('uLoginOptions'));
        if ($uopt['ulogin_check_mail'] == 0) return true;

        return Mail::Send(
            $this->context->language->id,
            'account',
            Mail::l('Добро пожаловать!'),
            array(
